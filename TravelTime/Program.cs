...
namespace TravelTime
{
    public class Program
    {
        ...
        private static char validateInstruction(char bearing)
        {
            char next = new char();
            switch (bearing)
            {
        ...
        public static void validateUserEntry(string[] instructions)
        {
            foreach (var instruction in instructions)
                validateInstruction(instruction[0]) ;

            for (int i = 0; i < instructions.Length - 1; i++)
            {
                // Validate 
                if (!instructions[i + 1][0].Equals(validateInstruction(instructions[i][0])))
                {
        ...
        public static int getNumberOfRightTurns(string[] instructions) => instructions.Length - 1;
    }
}
